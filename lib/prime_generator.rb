# Takes a number N of prime numbers to generate and exposes a #list_of_primes function to return them as a list.
# As an optimization, uses a basic prime sieve to indicate, for each found prime, that the multiples of that
# number are not prime (up to some limit) and do not need to be checked for primeness.
#
...
# The sieve is initialized as max_num_primes + 2 to satisfy the condition that max_num_primes == 1 requires 3
...
class PrimeGenerator
  ...
  def initialize(max_num_primes)
    ...
    #         0,     1,
    @sieve = [false, false]
    @max_sieve_size = max_num_primes + 2
    @populated = false
  end
  ...
  def generate_primes
    primes = []
    number_to_check = 0
    while primes.length < max_num_primes
      ...
      if sieve[number_to_check].nil?
        ...
        break if primes.length == max_num_primes
      end
      # If the number is prime, mark all multiples of it as not prime up to the maximum sieve size
      ...
      # already seen again, though, in order to correctly find primes in the new range size - size * 2.
